...
x = Conv1D(num_filters, 5, activation='relu')(embedded_sequences)
x = MaxPooling1D(5)(x)
x = Conv1D(num_filters, 5, activation='relu')(x)
x = MaxPooling1D(5)(x)
x = Conv1D(num_filters, 5, activation='relu')(x)
x = MaxPooling1D(5)(x)
x = Conv1D(num_filters, 5, activation='relu')(x)
...
for iteration in range(1, 21):
    ...
    model.save_weights(BASE_DIR + '/models/model_weights_' + str(iteration) + '_.h5')
    with open(BASE_DIR + '/models/model'+ str(iteration) + '.json','w') as model_json:
        model_json.write(model.to_json())


np.savetxt(BASE_DIR + '/results/tr_losses_'+ str(BATCH_SIZE) + '_' + OPTIMIZER + '_' + str(number_of_Dense_layers) + '.txt', tr_loss)
np.savetxt(BASE_DIR + '/results/val_loss_'+ str(BATCH_SIZE) + '_' + OPTIMIZER + '_' + str(number_of_Dense_layers) + '.txt', val_loss)
np.savetxt(BASE_DIR + '/results/tr_acc_'+ str(BATCH_SIZE) + '_' + OPTIMIZER + '_' + str(number_of_Dense_layers) + '.txt',tr_acc)
np.savetxt(BASE_DIR + '/results/val_acc_'+ str(BATCH_SIZE) + '_' + OPTIMIZER + '_' + str(number_of_Dense_layers) + '.txt',val_acc)
